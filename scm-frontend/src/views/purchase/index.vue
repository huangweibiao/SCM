  ...
  <div class="page-container">
    ...
    <el-card class="search-card">
      <el-form :model="queryParams" :inline="true">
        <el-form-item label="供应商ID">
          <el-input v-model="queryParams.supplierId" placeholder="请输入供应商ID" clearable @clear="handleQuery" />
        </el-form-item>
        ...
        <el-form-item label="订单编号">
          <el-input v-model="queryParams.poNo" placeholder="请输入订单编号" clearable @clear="handleQuery" />
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="table-card">
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="poNo" label="订单编号" width="180" />
        ...
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 1" type="info" link size="small" @click="handleReceipt(row)">收货</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 新增/编辑对话框 -->
    <el-dialog v-model="dialogVisible" title="采购订单" width="800px" @close="handleClose">
    ...
    <el-dialog v-model="detailDialogVisible" title="订单详情" width="900px">
      <el-descriptions :column="2" border v-if="currentRow">
        <el-descriptions-item label="订单编号">{{ currentRow.poNo }}</el-descriptions-item>
...
import { getPurchaseOrderPage, createPurchaseOrder, deletePurchaseOrder, auditPurchaseOrder, closePurchaseOrder, getPurchaseOrderDetail, getPurchaseOrderDetails } from '@/api/purchase'
...
const handleAudit = async (row) => {
  try {
    await ElMessageBox.confirm(`确定要审核订单"${row.poNo}"吗?`, '提示', { type: 'warning' })
    await auditPurchaseOrder(row.id, 1)
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(`确定要删除订单"${row.poNo}"吗?`, '提示', { type: 'warning' })
...
const handleClose = async (row) => {
  try {
    await ElMessageBox.confirm(`确定要关闭订单"${row.poNo}"吗?`, '提示', { type: 'warning' })
...
const handleReceipt = (row) => {
  ElMessage.info('收货功能在入库单中处理')
}

const getStatusText = (status) => {
  const map = { 0: '待审核', 1: '已审核', 2: '部分收货', 3: '已完成', 4: '已关闭' }
